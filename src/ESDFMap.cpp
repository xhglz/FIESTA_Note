...
// 使用DLLs可以在体素从占有变为空闲的时候，高效地更新ESDF地图
// 一个体素vox所对应的dll存放了所有最近障碍物是vox的体素
void fiesta::ESDFMap::DeleteFromList(int link, int idx) {
    // undefined_ = -10000;
    // link->idx->head 变成 link->head
...
bool fiesta::ESDFMap::UpdateOccupancy(bool global_map) {
    ...
    while (!occupancy_queue_.empty()) {
        ...
        // num_hit_ >= num_miss_- num_hit_ =>占据的次数 > 空闲的次数
        ...
        if ((log_odds_update >= 0 &&
            occupancy_buffer_[idx] >= clampm_ax_log_) ||
            (log_odds_update <= 0 &&
                occupancy_buffer_[idx] <= clamp_min_log_)) {
            continue;
        }
	    // 体素初始化为负无穷，初始化占据里的体素后设为正无穷
        ...
        // 更新占据概率 occupancy_buffer_: L' = L + p
        // min 和 max 增加范围限制
...
//    UpdateOccupancy();
    ...
    while (!insert_queue_.empty()) {
        ...
        if (Exist(idx)) {
            ...
            // closest_obstacle_ 最近障碍物的体素坐标
            // 双向链表: A的最近障碍物vox<->以vox为最近障碍物的体素
            ...
            for (int obs_idx = head_[idx]; obs_idx != undefined_; obs_idx = next_obs_idx) {
                ...
                for (const auto &dir : dirs_) {
                    ...
                    if (VoxInRange(new_pos) && closest_obstacle_[new_pos_idx](0) != undefined_
                        ...
                        // 比较:vox 是以删除的障碍物为最近障碍物的点,vox 到他邻居的最近障碍物的距离,小于他本来的距离(无穷或上一个邻居的距离)
                        // vox 到他邻居的最近障碍物的距离, 小于他本来的距离
                        if (tmp < distance) {
                            // 修改它到最近障碍物的距离
                            distance = tmp;
